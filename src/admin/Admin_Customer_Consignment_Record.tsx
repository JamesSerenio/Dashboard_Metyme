...
import "../styles/Admin_Customer_Consignment_Record.css";
...
const Admin_Customer_Consignment_Record: React.FC = () => {
  ...
  const [anchorDate, setAnchorDate] = useState<string>(() => todayPHKey());
  ...
  const anyModalOpen =
    !!selectedOrder || !!paymentTarget || !!voidTarget || !!cancelTarget || deleteRangeOpen;
    ...
    try {
      ...
      for (let idx = 0; idx < filtered.length; idx++) {
        ...
        row.eachCell((cell, colNumber) => {
          cell.alignment = {
            vertical: "middle",
            horizontal: colNumber === 2 || colNumber === 4 || colNumber === 15 ? "left" : "center",
            wrapText: true,
          };
          cell.border = {
        ...
        if (imageColIndex > 0 && isLikelyUrl(imgUrl)) {
          ...
          if (ab) {
            const ext =
              imgUrl.toLowerCase().includes(".jpg") || imgUrl.toLowerCase().includes(".jpeg")
                ? "jpeg"
                : "png";
            const imgId = wb.addImage({ buffer: ab, extension: ext });
          ...
          <div className="customer-topbar-right">
            ...
            <div className="admin-tools-row">
              <label className="date-pill">
                <span>Mode</span>
                <select value={filterMode} onChange={(e) => setFilterMode(e.currentTarget.value as FilterMode)}>
              ...
              <div className="date-pill">
                <span>{filterMode === "day" ? "Date" : "Anchor"}</span>
                <input
                  type="date"
                  value={anchorDate}
              ...
              <button
                ...
                onClick={() => void fetchByRange(activeRange.startKey, activeRange.endKey)}
                disabled={loading}
              >
                Refresh
              </button>
                      ...
                      <td style={{ width: 86 }}>
                        {img ? (
                          <img
                            src={img}
                            alt={itemName}
                            className="consignment-thumb"
                            loading="lazy"
                          />
                        ) : (
                          <div className="consignment-thumb-placeholder">No Image</div>
                        )}
                      </td>

                      <td className="td-strong">{itemName}</td>
                      <td className="td-strong">{cat}</td>
                      <td>{formatPHDateTime(r.created_at)}</td>
                      <td className="td-strong">{show(r.full_name)}</td>
                      <td className="td-strong">{show(r.seat_number)}</td>
                      <td>{sizeText(r.consignment?.size)}</td>

                      <td className="td-strong">{qty}</td>
                      <td className="td-strong nowrap">{moneyText(price)}</td>
                      <td className="td-strong nowrap">{moneyText(total)}</td>
        ...
        {deleteRangeOpen && (
          <div className="receipt-overlay" onClick={() => (deletingByRange ? null : setDeleteRangeOpen(false))}>
            <div className="receipt-container" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="receipt-row receipt-row-small">
                <span>Warning</span>
          ...
          <div className="receipt-overlay" onClick={() => (cancelling ? null : setCancelTarget(null))}>
            <div className="receipt-container" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-hint">
                • This will be saved to <b>consignment_cancelled</b> then removed from{" "}
                <b>customer_session_consignment</b>.
                <br />
                • If this record is <b>NOT VOIDED</b>, stock will be returned by reducing{" "}
                <b>consignment.sold</b>.
              </div>
                  ...
                  <>
                    ...
                    {isVoided && (
                      <>
                        ...
                        <div className="receipt-row">
                          <span>Void note</span>
                          <span className="right-note">{show(selectedOrder.void_note, "-")}</span>
                        </div>
                      </>
                    )}
...
export default Admin_Customer_Consignment_Record;